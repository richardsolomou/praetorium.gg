...
test('Death Guard champions expose their legal wargear', async ({ page }) => {
  ...
  await expect(deathshroudChampion.getByText('Icon of Despair (Aura)', { exact: true })).toHaveCount(1)
  await expect(loadout.getByText('Wargear options', { exact: true })).toHaveCount(0)
