...
for (const width of [1440, 390, 860, 1024]) {
  test(`automatic shooting and melee with stable edits at ${width}px`, async ({ page }) => {
    ...
    for (const results of [shooting, melee]) {
      await expect(results.getByRole('heading', { name: 'Models destroyed', exact: true })).toBeVisible()
      await expect(results.getByRole('heading', { name: 'Wounds lost', exact: true })).toBeVisible()
...
for (const width of [1440, 390]) {
  test(`printed Feel No Pain applies, resets and swaps with the unit at ${width}px`, async ({ page }) => {
    ...
    await expect(
      page.getByRole('region', { name: 'Defender rules', exact: true }).getByRole('heading', { name: 'Feel No Pain 4+', exact: true }),
    ).toBeVisible()
    await expect(shooting).toHaveAttribute('aria-busy', 'false')
