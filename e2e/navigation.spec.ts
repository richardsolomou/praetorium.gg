...
test('a player can enter through the roster library and browse the product', async ({ page }) => {
  ...
  await expect(page.locator('[data-shelf="Xenos"] [data-faction]').first()).toHaveAttribute('data-faction', 'Necrons')
  ...
  const xenos = page.getByRole('group').filter({ has: page.getByText('Xenos', { exact: true }) })
  await expect(xenos.getByRole('option').first()).toHaveText('Necrons')
  await page.keyboard.press('Escape')
  await page.keyboard.press('Escape')
  await page.getByRole('link', { name: 'Factions' }).click()
