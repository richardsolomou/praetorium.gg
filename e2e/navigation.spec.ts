...
test('a player can enter through the roster library and browse the product', async ({ page }) => {
  ...
  const beforeError = await setup.boundingBox()
  await setup.getByRole('button', { name: /Cryptek Conclave/ }).click()
  await expect(setup.getByRole('alert')).toHaveText('5 used / 2 available DP')
  await expect(setup.getByRole('button', { name: 'Done' })).toBeDisabled()
  expect(await setup.boundingBox()).toEqual(beforeError)
  await page.screenshot({ path: 'test-results/edit-roster-setup-error.png', fullPage: true })
  await setup.getByRole('button', { name: /Cryptek Conclave/ }).click()
  ...
  await page.getByRole('button', { name: 'Roster actions' }).click()
  await page.getByRole('menuitem', { name: 'Edit roster setup' }).click()
  await setup.getByRole('button', { name: /Awakened Dynasty/ }).click()
  await setup.getByRole('button', { name: /Cursed Legion/ }).click()
  await setup.getByRole('button', { name: 'Done' }).click()
