...
test('a player can enter through the roster library and browse the product', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Reanimation Protocols' }).hover()
  await expect(page.getByRole('tooltip')).toContainText('Command phase')
  ...
  await page.getByRole('button', { name: 'Leader', exact: true }).first().hover()
  await expect(page.getByRole('tooltip')).toContainText('Before the battle')
...
test('a dense squad datasheet remains readable at desktop and phone widths', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Models 2', exact: true })).toBeVisible()
