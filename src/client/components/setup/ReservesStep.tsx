      ...
      {sections.map((section) => (
        <section key={section.label} className="space-y-1">
          ...
          {section.units.map((unit) => (
            <UnitFormationRow key={unit.key} army={army} unit={unit} send={send} />
          ))}
        </section>
      ))}
    </article>
 ...
 */
function UnitFormationRow({ army, unit, send }: { army: Army; unit: Army['units'][number]; send: (command: Command) => void }) {
