...
export const commandSchema: z.ZodType<Command> = z.discriminatedUnion('kind', [
  ...
  z.object({ kind: z.literal('draw-secondary'), secondary: z.object({ key: id, name: z.string().min(1).max(ROSTER_NAME_MAX_LENGTH) }) }),
  z.object({ kind: z.literal('select-secret'), secondary: z.object({ key: id, name: z.string().min(1).max(ROSTER_NAME_MAX_LENGTH) }) }),
