    ...
    handlers: {
      GET: ({ params }) =>
        previewResponse(async () => {
          ...
          const screen = await instance.service.screen(params.token, null, await instance.rulesFor()).catch((error: unknown) => {
            if (error instanceof Response && error.status === 404) return null
            throw error
          })
          if (screen?.kind !== 'spectator') return null
          const factions = new Map(((await instance.factionIndexFor())?.factions ?? []).map((faction) => [faction.id, faction.displayName]))
          return {
