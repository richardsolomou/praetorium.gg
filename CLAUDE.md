...
- **The points ratchet is a ratchet.** `pnpm catalogue:points` agrees with the Munitorum on 99.6% of 1,863 checks. A change that lowers it is a regression even if every unit test passes — unless what the check _builds_ changed, in which case say so and rebaseline.
