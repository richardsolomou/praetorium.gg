...
- **League sealing is a legality boundary.** Enforce global rules the catalogue may not express when the snapshot is sealed: every roster outside 2v2 has exactly one eligible Warlord, while a 2v2 team has exactly one between its two rosters. Reveal revalidates team-wide facts as a safety check.
