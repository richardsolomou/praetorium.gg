...
- **Each deployment has one authoritative storage topology.** Self-hosted deployments use Postgres for accounts and product data; `VALKEY_URL` is optional and holds sessions, the auth limiter, and Centrifugo's engine for multiple replicas. The hosted topology uses D1 for accounts and SpacetimeDB for product data and realtime updates. Back up both authoritative stores and the auth secret together. Every repository method is async; batch Postgres reads in `src/db/repository.ts`.
