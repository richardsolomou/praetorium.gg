...
Live state for one game of Warhammer 40,000 between opposing sides of up to three players, and the list building that feeds it. No rules encyclopedia, no unit-level wound tracking, nothing social, and **no game data in this repository** — an instance fetches the community catalogues itself.
...
- **`src/core` is the domain and stays free of IO and framework imports.** `battle.ts` is the whole game; `evaluate.ts`, `roster.ts` and `catalogue.ts` are the whole catalogue. Oxlint enforces the import boundary.
