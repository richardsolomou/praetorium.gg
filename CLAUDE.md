...
- **A name is not a battle.** A player's profile is open to anybody; what `battleAudience` withholds is their games. Gating a name that is already on every watchable battle only made links that led nowhere.
- **`battleAudience` alone decides who may watch a battle.** A player answers once, in `battle_sharing`, and a battle takes the narrowest answer of everyone seated in it — so one player saying private makes the battle private for the table. The home-page feeds and the screen a link resolves to are the same question and must both read that one answer. Absence of a row is public: the default lives in `src/core/battleAudience.ts` and nowhere else. Watching is read-only and never claims a seat.
...
- **The points ratchet is a ratchet.** `just points` matches 100% of the generated reference checks. A lower result is a regression unless the generated check set changed and the new baseline is explained. A pinned baseline fails a pull request, where one snapshot measured against both revisions makes a moved number ours, and only reports on the snapshot publisher, which has no such comparison and would otherwise hold the whole refresh back whenever two sources disagree about a name.
