...
| Matching BattleBase                        | [docs/parity.md](docs/parity.md)                                                                             |
...
- **No game data lives in this repository, ever.** `catalogue/sources.json` records where it comes from and pins a revision; `catalogue-data/` is fetched and gitignored. This is what lets the project be public without redistributing Games Workshop's content. The rule was never "no stratagems" — it is "nothing vendored".
...
- **Public documentation is portable.** Explain the product, contribution workflows and self-hosting requirements, but keep provider-specific infrastructure, hostnames, credentials and details of the maintained deployment out of the repository.
- **The points ratchet is a ratchet.** `just points` agrees with the Munitorum on 99.6% of 1,863 checks. A change that lowers it is a regression even if every unit test passes — unless what the check _builds_ changed, in which case say so and rebaseline.
