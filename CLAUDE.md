...
Live state for one game of Warhammer 40,000 between opposing sides of up to three friends, the list building that feeds it, and private roster-reveal events before matchups are decided. Social scope stops at mutual friendships used to form private battles and events: no chat, feeds, public groups, or public discovery. No rules encyclopedia, no unit-level wound tracking, and **no game data in this repository** — an instance fetches the community catalogues itself.
...
- **`src/core` is the domain and stays free of IO and framework imports.** `battle.ts` is the whole game — the log, `validate` and `apply` — with `battleView.ts` deciding what a player may see and `battleReport.ts` putting the log into English. `catalogue.ts` and `evaluate.ts` read and price the community data, and `roster.ts` turns a saved pick back into a unit with `definitions.ts`, `selection.ts`, `expand.ts`, `unitSize.ts`, `unitChoices.ts`, `unitSpread.ts`, `modelKinds.ts` and `wargear.ts` beneath it. Oxlint enforces the import boundary.
...
- **The points ratchet is a ratchet.** `just points` matches 100% of 1,861 reference checks. A lower result is a regression unless the generated check set changed and the new baseline is explained.
