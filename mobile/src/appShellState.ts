...
export function webLoadFailed(state: AppShellState): AppShellState {
  return { ...restoreDelivery(state), ready: false }
}

export function webLoadSucceeded(state: AppShellState, url: string): AppShellState {
  return { ...webNavigationChanged(state, url), ready: true, delivering: null }
}
...
export function authDeliverySucceeded(state: AppShellState): AppShellState {
  ...
  return {
    ...state,
    delivering: { kind: 'navigation', url: new URL(state.delivering.callback.next, APP_URL).href },
  }
}

export function rendererTerminated(state: AppShellState): AppShellState {
  const restored = webLoadFailed(state)
  return {
    ...restored,
