...
class DetectCloudflareCutoverTest(unittest.TestCase):
    def run_detection(self, routes: list[dict[str, str]], runtime: bool, pages: int | None = 1) -> subprocess.CompletedProcess[str]:
...
""")
            ...
            env = {
                **os.environ,
                ...
                "MOCK_ROUTES": json.dumps({"success": True, "result_info": {"total_pages": pages} if pages is not None else None, "result": routes}),
    ...
    def test_first_cutover_has_no_routes(self) -> None:
        result = self.run_detection([], False, pages=None)
    ...
    def test_unpaginated_full_page_fails_closed(self) -> None:
        result = self.run_detection([{"pattern": f"example{i}.gg/*", "script": "other"} for i in range(100)], False, pages=None)
        self.assertNotEqual(result.returncode, 0)
