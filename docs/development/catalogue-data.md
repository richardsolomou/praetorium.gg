...
- Show a named core ability granted to a bearer alongside its other core abilities, and one granted to every model in an attached unit as an inherited ability. Preserve the granting rule as their source. Take the ability name and parameter from a structured rule link when one exists; recognize only exact catalogue phrases for its recipient or as a fallback when no link exists. Conditional or unfamiliar prose stays as prose rather than being guessed.
