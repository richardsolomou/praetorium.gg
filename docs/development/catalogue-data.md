...
- A choice a text export states that its unit cannot be given is named back per unit: equipment and enhancements the datasheet cannot place, a Warlord it cannot be, a squad size it does not field, and an attachment whose target the list does not hold. Each says why, because the fix is somewhere different for each: an enhancement names the detachment that offers it, wargear separates a name the faction does not have from one this datasheet is not offered, and a squad size names the sizes the datasheet fields. Equipment is unplaced only when it matches no choice option, nothing the built unit holds, and no model in it, so fixed wargear and the model names an export lists beside their weapons are silent. A squad size counts only where the export states one.
- An import saves whatever it could read and writes what it could not into `rosters.import_notes`. Only a faction nobody recognises refuses outright, because there is no book to build from; a unit that matched nothing is skipped and named, and the rest of the list still arrives. The unit keeps what its datasheet builds, because substituting a choice nobody asked for is the answer a player cannot correct at the table.
- `import_notes` is written only by the insert that an import performs and never by a later save, so the builder's autosave cannot wipe it; `clearImportNotes` drops it once the player has read it. It reaches the owner alone, through the same `owner` gate on `rosterFromRow` that withholds their prep. The builder says it twice and never the same way: the unit card carries the detail, keyed by `entryId` so a display name cannot mismatch, and the footer carries what has no card — a unit that never arrived — plus the count of the rest.
