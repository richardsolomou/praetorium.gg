...
- Enhancement names and points come from the rules source; `describedEnhancements` is the one place their text is resolved, preferring the catalogue, then Game Datacards, matched by detachment and name with any `(Aura)` or `(Upgrade)` suffix ignored; leave conflicting matches blank.
