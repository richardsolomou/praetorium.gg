...
- The picker, roster and faction datasheet list shelf each datasheet by its primary category. Unknown or missing primary categories stay under Other rather than being inferred from secondary keywords.
...
- `set-primary` and `unset-primary` stay unhandled on purpose, and the census says so. They name the force slot that holds a selection, which is not a question this app asks: most of them set an `Allies: <faction>` category that shelves nothing here, and the rest re-shelve a unit for one detachment. A shelf that moved with the roster would also break the one thing `groupOfEntry` exists to guarantee — that the picker and the roster sort a unit the same way.
