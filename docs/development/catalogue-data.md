...
- Key the rules dataset by its faction directory and by every alias that directory declares. The catalogues call the Adeptus Astartes book Space Marines, and a lookup by the name a player sees must still find its detachment and stratagem semantics.
...
- Show only detachments named by the faction's Game Datacards file or its declared parent faction's file. A catalogue import does not make another faction's detachments its own, and stale rules-source names are reported rather than offered.
- A detachment's stratagems are the union of the ids it names and the records filed under it. A stratagem is named as its card prints it; the rules dataset shouts, and only a stratagem without a card is title-cased from that. The dataset writes a shared stratagem down once and the other detachments reach it by id only; a card reached both ways is kept once, as the copy filed under this detachment.
...
- Homebrew is picked, never assumed. `OPTIONAL_RULE_IDS` is the mirror of `FORMAT_RULE_IDS`: a format rule is played until a roster waives it, a optional rule is not played until a roster picks it. `optionalRules` names what a size offers, a roster carries the ids it picked, and `pickedOptionalRules` is the one answer to what a list is playing beyond its format. They are chosen from their own dialog rather than roster setup, so setup stays about the army and the list of them can grow. Add a optional rule here when you add homebrew, so nobody is handed a rule their group did not agree to play.
...
- A battle size's restrictions are overridable per roster. `formatRules` names every restriction a size imposes, a roster carries the ids it has waived, and each waiver is read in both directions: nothing waived is reported by the price, and nothing waived filters the picker. They are switched off from the picker's own menu, over the book they act on, and what a list has switched off is warned about under the roster beside the legality errors, because a restriction switched off is the reason an error a player expected is not reported. That warning is dismissible and returns when the set of waived restrictions changes. Add a restriction to `formatRules` when you add one to a format, so a player can switch it off and the two sides cannot disagree about which rules are being played. A waived roster stays usable — it attaches to a battle and carries its waivers into the snapshot, so the battle prices it under the rules it was built under. It also declares them: `waivedFormatRules` is the one answer to what a list is not playing, drawn on the list in the library and the builder, on the row being chosen for a seat or sealed into a league, and on the army once every seat can see it, behind a confirmation at the moment it stops being the owner's own business.
