...
- Detachment points, faction overrides, force dispositions and enhancement points come from Game Datacards. Join them to the rules source by exact detachment and card names, with any `(Aura)` or `(Upgrade)` suffix ignored; leave missing or conflicting numbers blank and report the join in `just points`.
- Enhancement names come from the rules source; `describedEnhancements` is the one place their text is resolved, preferring the catalogue, then Game Datacards, matched by detachment and name with the same suffix handling; leave conflicting matches blank.
