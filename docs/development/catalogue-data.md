...
That covers agreement between sources and nothing else. An unresolvable upstream, an incomplete snapshot, and an archive that does not verify after upload all still fail the publisher, and a failed publish opens or comments on one issue rather than leaving stale data as the only signal.
