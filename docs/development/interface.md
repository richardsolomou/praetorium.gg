...
- Explain everything on hover through `HoverTooltip`, which takes a title, the words themselves, and a note saying where they came from. The shape is the component's API rather than free-form content, so a keyword's rule and a modified characteristic read alike; the generated Base UI tooltip owns positioning, focus, collision handling, and scroll tracking.
