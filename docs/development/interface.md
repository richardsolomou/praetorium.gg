...
- Word the prompt as the card does: its own text, then one row per condition with what meeting it pays, and a row for scoring nothing. The payout itself is the control, so the number pressed is the number scored. Rows the card puts in one group are tiers, so picking one clears the others; anything ungrouped a card can pay at the same time. A counted payout is a count, bounded by the ceiling the card sets.
