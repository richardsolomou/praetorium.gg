...
Each ability in an editable roster has a bell that opens its alert settings. One alert can have several independent triggers, and every trigger chooses its own start or end moment, phase, and turn scope; for example, an ability can alert in the player's Shooting phase and in either player's Fight phase. Catalogue prose supplies editable suggestions when it can. The bell in the roster header enables or disables all configured alerts without deleting them. Removing a unit removes its alerts, while army-wide faction abilities are stored once for the roster.
...
The live tracker groups every personal ability alert due at the same moment into one dialog. Each alert in the group can be dismissed for the current phase, turn, round, or battle without dismissing the others. Start alerts appear when that phase or turn is entered; end-of-phase alerts appear before the player's phase-control action continues, and end-of-turn alerts appear in the end step. Shared scoring, draw, discard, and Secret Mission dialogs wait until that personal reminder closes.
