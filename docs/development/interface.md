...
A signed-in player sees a short field guide on first use. Its non-modal task panel stays out of the way of the application it introduces. The guide teaches the path from creating a roster and adding a friend to starting a battle by taking the player to the real control for each task. It also tours organized play; global search, factions, datasheets, mission matchups, and source-rule sections; and the home feed, standings, profiles, and battle visibility. Related reading surfaces stay grouped into one reference tour rather than becoming a task for every page, but the tour follows the player from each index into representative detail pages. A workflow stays guided after its first button: roster creation explains the setup dialog, unit picker, roster, and loadout; battle creation explains how to seat the table; and league creation continues from registration settings into the event workspace. Roster progress is earned after a unit is added, and league progress after the player creates or joins one. The guide never blocks normal use, keeps progress with the account, allows irrelevant tasks to be skipped, and remains available from the account menu after completion.
...
Each ability and selected enhancement or unit upgrade in an editable roster has a bell that opens its alert settings. One alert can have several independent triggers, and every trigger chooses its own start or end moment, phase, and turn scope; for example, an ability can alert in the player's Shooting phase and in either player's Fight phase. Catalogue prose supplies editable suggestions when it can. The bell in the roster header enables or disables all configured alerts without deleting them. Removing a unit removes its alerts, changing a selected enhancement or upgrade removes the alert for the old selection, and army-wide faction abilities are stored once for the roster.
...
The owner's View mode keeps the picker column and compact Add units button available while withdrawing the unit-card menus and loadout controls. What the datasheet says about a unit — its attachments, the units it can lead or be led by — is a fact about the unit rather than an edit, and stays visible in every reading. Other readers do not get the picker or any mutation controls.
...
The live tracker groups every personal roster alert due at the same moment into one dialog. Each alert in the group can be dismissed for the current phase, turn, round, or battle without dismissing the others. Start alerts appear when that phase or turn is entered; end-of-phase alerts appear before the player's phase-control action continues and after a different device advances the table, and end-of-turn alerts appear in the end step. Entering the Shooting phase also includes the actions on that player's active primary and secondary mission cards. Mission action reminders default on and can be disabled for that battle from Battle options. Shared scoring, draw, discard, and Secret Mission dialogs wait until that personal reminder closes.
