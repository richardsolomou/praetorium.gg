...
- `src/db` owns Postgres schema, migrations, and repositories.
- `src/adapters` owns external transports such as email, realtime, telemetry, and Valkey.
- `src/server` owns application services, catalogue loading, authentication, and server functions.
...
Tests stay beside the behavior they specify. Split large suites by scenario while continuing to exercise the same public entry point.
