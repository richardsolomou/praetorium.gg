...
- A mission card's instructions come from the pack's `description`, including any choice made when the card is drawn. Its `lore` is not shown.
...
- The action a card names comes from that same pack, joined to the card by name, because the rules source describes no action. Every line is the pack's own sentence, a line it does not state is absent, and a card name two packs disagree about carries no action rather than the wrong one. Those same structured actions produce the optional reminder when their side enters the Shooting phase; cards without a joined action produce none.
...
- What their turn owed comes before the hand this one deals. Both prompts block battle actions, so only one is shown at a time; compact application navigation remains available outside them.
- Tactical draws can be random or selected from the remaining deck. Manual selection is available at every draw so a player can recreate a physical hand after undoing a mistaken action without swapping a card accidentally. `when_drawn` defines when a card can or must return. The battle enforces mandatory `battle_round` returns before the turn can continue, offers paired `card_ids` returns as optional, and leaves board-state conditions for the player to judge.
