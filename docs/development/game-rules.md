...
- A tactical secondary is played once: scoring it finishes it. An unresolved tactical card is only discarded when its owner chooses to at the end of the turn — it is never discarded automatically, and can be kept into the next turn instead. The hand fills back to two at the top of the next turn. A fixed hand is chosen for the whole battle and is not finished by being scored.
