...
The leaderboard and a player's profile carry no tab: the application is for playing at the table, and neither is a page anybody opens mid-game. Both are still named in the top bar, because a rank links to one and a name links to the other. Each tab returns to the screen it was left on. The session records the last location, history state, and scroll positions of every section in `src/client/nativeTabs.ts`, keyed by the section `nativeNavigation` derives from the path. This keeps an open roster unit open when another tab is visited. Tapping the section you are already in goes to its top instead. The record lives in `sessionStorage`, so a cold start opens every tab at its top.
