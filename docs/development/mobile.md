...
The iOS and Android application uses an Expo shell around the existing responsive interface. React Native owns the safe areas, header, tabs, system authentication, sharing, printing, haptics, and screen wake locks. The server remains authoritative for accounts, rosters, and battles.
...
The shell declares the `app-navigation` capability and marks each document as an installed-application document. React Native owns the fixed top bar and five primary tabs while the responsive application renders beside them. Tabs form a bottom bar in portrait and a left navigation rail in landscape. More is a native navigation screen for Leagues, Leaderboard and Rules. The embedded document hides both forms of website chrome, and browser visitors keep the normal responsive header.

The leaderboard and a player's profile carry no tab: the application is for playing at the table, and neither is a page anybody opens mid-game. The native header still gives these pages a stable frame. Tab presses send trusted section URLs to the existing WebView, while in-section links retain browser history and state.
...
GitHub builds the iOS application with Fastlane and publishes its JavaScript updates with EAS Update. Android builds use `mobile/eas.json`. Apple metadata lives in `mobile/store.config.json`; Google Play metadata and both stores' manual review fields are recorded in [Mobile release](mobile-release.md).
...
The GitHub workflow builds the canary and stable iOS applications in sequence on macOS 26. It always builds canary first and stable second. Fastlane assigns each build the next TestFlight number, signs it with an App Store Connect API key, uploads it, and waits for processing. EAS Update then publishes the same merged revision on the matching channel for compatible installed builds. A running delivery finishes before the next delivery starts. GitHub replaces an older queued delivery when a newer mobile revision reaches `main`. An open pull request only runs validation. Android builds, updates, and store uploads remain manual. Android delivery stays disabled until the physical-device release gate and Google account requirements are complete.
