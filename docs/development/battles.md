...
`src/core/serviceRecord.ts` folds those battles into that player's record: totals, the win rate split by whether their side took the first turn, average victory points overall and in wins and losses, the margin they lose by, the primary and secondary halves, and win streaks. It reads `battleOutcome` and `sideScore` from `standings.ts`, so a profile cannot decide a battle differently from the table that ranks it, and `winRate` takes the counts rather than a row for the same reason. `recordFacets` offers only the factions, detachments, opponents, mission packs and battle sizes that player's own battles hold, and the narrowing rides in the address so a filtered record is a link. Practice games are absent, as they are from the home page and the leaderboard. `PraetoriumService.playerRankings` reads the leaderboard's own unsliced fold, so a rank on a profile is the row that page would print.
