...
Roster reminders for abilities, enhancements, and unit upgrades freeze with that roster snapshot. `battleView` returns them only to the player who owns the roster; opponents and spectators never receive them, and shared or revealed league-roster reads strip them too. The client also derives Shooting-phase reminders from the active primary and secondary cards when their fetched references name actions. Both kinds are optional personal prompts rather than commands, so dismissing one neither writes to the battle log nor blocks another seated device. A player can dismiss each grouped reminder for the current phase, turn, round, or the rest of the battle; those choices persist in one bounded local record per battle without revealing them to the opponent. Mission action reminders have a separate per-battle device preference, default on, under Battle options. The client does not infer that a destroyed unit makes its reminder irrelevant because delayed effects and rules that outlive their source make that unsafe. The client shows each start trigger once per battle moment, intercepts its own phase advance for an end trigger before continuing the ordinary shared prompt sequence, and notices the previous phase ending when another device advances the table.
...
During setup, each catalogue-backed army can place at most half its battle-size points limit in Strategic Reserves. Deep Strike units count towards the same limit because the ability changes their ingress rather than removing them from Strategic Reserves. Source-declared exemptions and units redeployed after deployment do not count. After the first-turn roll, returning a battlefield unit to Strategic Reserves through the Reserves section records that redeployment. The limit and its catalogue-derived exemptions freeze with the roster snapshot when every selected detachment and enhancement joined to complete rules detail; snapshots without complete facts remain startable rather than guessing. The limit is checked when a formation changes and again when the battle begins; moves into Strategic Reserves after the battle begins are not starting formations and do not consume it.
...
A side is one stratagem pool, so a 2v1 pools both allies' detachments into it, keyed by the dataset's own stratagem id. `set-prep` targets the side captain, while any seated device can record the derived pool, choose fixed or tactical play, and select fixed cards for either side. `sideStratagems` in `src/client/features/battle/sideRules.ts` assembles the pool, and `armyRulesRequest` beside it is the one derivation of what an army's rules are looked up by.
...
Link previews ask `screen` the same question. A crawler carries no session, so the battle page it fetches is built from the answer a signed-out visitor gets, and the preview image under `/api/previews/battles/$token` asks with no viewer at all, whoever requests it. A spectator screen previews as its players, armies, stage and score, read from the spectator view so a face-down Secret Mission cannot reach it. `unavailable` previews as a generic battle with the instance's card, and its image is a `404`. A battle opened from a league event is watchable by anyone once it exists, so it previews whatever its players' own answers. [Interface](interface.md#link-previews) covers the tags and the image.
...
Rows are ordered by wins, then the win rate, then wins less losses, then the victory points behind them. Wins and the rate are both columns the table prints, so the order can be checked against the row. The cost is that this rewards turning up: eleven wins from twenty battles outranks nine from nine. The rate keeps a single lucky game off the top, and wins less losses separates two players who match on both. A draw counts half in the rate rather than as a loss. Games of every table shape count together: a side is the unit that scores everywhere else in the product, and splitting a small instance's few battles across shapes would produce tables that each answer nothing.

The fold reads whole logs, so it is bounded by a window and a count, and the service holds the answer for a minute. That is allowed here and nowhere near a battle screen: a stale standing costs a reader a minute of accuracy, while a stale battle is a player acting on a board that has moved. The battle limit can bite before the ninety-day window does, so the answer reports the days it actually reached back rather than the ninety it asked for. Each table sends its leading rows and its full player count, because the payload is one table per faction played.
...
The profile page is open to the same extent, split across tabs for the record, the battles and the published rosters. It lists the battles this reader may watch that player in, narrowed by `PraetoriumService.watchable` — the batched form of the fold a battle link reads, so a profile can never list a battle that link would refuse. `Repository.battlesSeatedBy` narrows in SQL only to what could ever be shown to this reader; the decision stays in `battleAudience` and `maySpectate`, and a viewer holding a seat always sees their own battle. A player who keeps their battles private has a profile with a name on it and nothing else, and its link preview says no more.

`src/core/serviceRecord.ts` folds those battles into that player's record: totals, the win rate split by whether their side took the first turn, average victory points overall and in wins and losses, the margin they lose by, the primary and secondary halves, and win streaks. It reads `battleOutcome` and `sideScore` from `standings.ts`, so a profile cannot decide a battle differently from the table that ranks it, and `winRate` takes the counts rather than a row for the same reason.

The same fold counts what the player's side did with its resources. `seatPlays` reads each seat's command points spent, stratagem uses, held secondary cards and primary mission off the state `reduceBattle` already folded for the battle list, so an undone command was never there to count and no log is read twice. A stratagem use records its name and the CP actually paid when it happens, including a price the board raised. The record prints the stratagems used most with their uses and CP, the CP spent per battle, and victory points per CP spent, which is the side's points over its spend and absent when it spent none. A secondary card counts as held in a battle unless it was put back the moment it was drawn; its row prints how often it was held, how often it scored anything and its points per battle held. The best and worst cards are read only among cards held at least three times, and only once two cards qualify. The primary missions and the armies faced each print played, won, drawn and lost, the missions with average victory points and the armies with the win rate; a 2v2 against two of the same army counts once, and a pasted list names no army. Every table is ordered by columns it prints.

Allies share a side's command points, stratagems and cards the way they share its score, so each ally is credited with the side's whole spend, every stratagem it used and every card it held — `sidePlay` adds the side's seats together exactly as `sideTotal` adds its points. A faction filter therefore still shows the ally's detachment stratagems in a 2v1, because the side used them. A face-down Secret Mission the reader may not name is left out of the card table altogether, through the same `mayNameCard` that `battleView` masks by; its points stay in the secondary total, which the battle already shows.

Names link to the page printing them only where that page answers: a detachment stratagem whose detachment the fielding army's faction lists, a secondary card the rules carry in a pack they carry, and a primary whose matchup page resolves. The primary is the one the battle screen resolves from both sides' dispositions. Core stratagems have no page and stay unlinked. `recordFacets` offers only the factions, detachments, opponents, mission packs and battle sizes that player's own battles hold, and the narrowing rides in the address so a filtered record is a link. Practice games are absent, as they are from the home page and the leaderboard. `PraetoriumService.playerRankings` reads the leaderboard's own unsliced fold, so a rank on a profile is the row that page would print.
...
`src/core/battleAudience.test.ts` covers the fold from seats to an audience, `src/core/standings.test.ts` covers the table folded from finished battles, and `src/core/serviceRecord.test.ts` covers one player's record, its narrowing and its facets. `src/server/service.integration.test.ts` covers persistence, deletion permissions, seating either side of a 2v1, concurrent submissions, and who each feed and each link answers, against an in-process Postgres. `e2e/home-activity.spec.ts` drives the home page from a signed-in player and a signed-out visitor at once and proves the link stops answering when the player withholds it. `src/client/sides.test.ts` covers the fold from seats to sides, `src/client/features/battle/sideRules.test.ts` covers the stratagem pool a side plays with, and `e2e/team-battle.spec.ts` drives a 2v1 from each side of it to prove the allied pair shares one pool.
