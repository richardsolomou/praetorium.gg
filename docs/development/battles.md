...
Names and pictures are public; battles are visible only through `battleAudience` and `maySpectate`. The profile uses `PraetoriumService.watchable`, the batched version of a battle link's visibility check, so its record, battle list, and rank cannot reveal a withheld game. SQL narrows candidates, but does not decide access. Roster visibility follows its own sharing rules.

`src/core/serviceRecord.ts` folds the visible battles into totals, outcomes, mission and opponent breakdowns, and resource use. It shares `battleOutcome` and `sideScore` with standings and reads already folded command state, so undone actions do not count. Allies share their side's score, command points, stratagems, and cards. A face-down Secret Mission stays out of named card results even when its points contribute to the visible total. Practice battles are absent from profiles' records and rankings.
