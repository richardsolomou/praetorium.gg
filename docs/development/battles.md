...
Losing models is a command like any other. `wound-unit` takes models off a unit one at a time and `set-unit` takes the whole unit, and the two never disagree: losing the last model is losing the unit, and a unit brought back is whole again. Both are in the report and both are undoable, and either seated player may record either army's losses — nothing about a unit is hidden from anyone.
