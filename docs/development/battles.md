...
Ability reminders freeze with that roster snapshot. `battleView` returns them only to the player who owns the roster; opponents and spectators never receive them, and shared or revealed league-roster reads strip them too. They are optional personal prompts rather than commands, so dismissing one neither writes to the battle log nor blocks another seated device. A player can dismiss each grouped reminder for the current phase, turn, round, or the rest of the battle; those choices persist locally without revealing them to the opponent. The client does not infer that a destroyed unit makes its reminder irrelevant because delayed effects and rules that outlive their source make that unsafe. The client shows each start trigger once per battle moment and intercepts its own phase advance for an end trigger before continuing the ordinary shared prompt sequence.
