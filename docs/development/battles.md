...
A side is one stratagem pool, so a 2v1 pools both allies' detachments into it, keyed by the dataset's own stratagem id. `set-prep` targets the side captain, while either seated device can record the derived pool so setup does not wait for its player to open the section. The side writer alone chooses fixed or tactical play and any fixed cards. `sideStratagems` in `src/client/sideRules.ts` assembles the pool, and `armyRulesRequest` beside it is the one derivation of what an army's rules are looked up by.
