...
Each pull request except an automated release candidate gets a disposable preview. The Worker build and deployment are separate jobs. A preview is marked ready only after its application reports the expected commit from the health endpoint.
